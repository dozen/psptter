...
function makeRequest(value, count, sendtype) {
    ...
    request.send(sendtype + '=' + value);
}

function tweetStatus() {
    text = document.getElementsByName('tweet')[0].value;
    tweetID = document.getElementsByName('id')[0].value;
    if(typeof window.XMLHttpRequest != 'undefined') {
        try {
            request = new XMLHttpRequest();
        } catch(err) {
